...
def tarball_hashes(package, version, url):
    """The SHA-256 of every file in a release, keyed by its path inside the package."""
    ...
    try:
        with tarfile.open(archive_path) as archive:
            ...
            return {
                member.name.split("/", 1)[-1]: sha256(archive.extractfile(member).read())
...
def identify(target, packages, prefix):
    wanted = sha256(target.read_bytes())
    print(f"{target}\n  sha256 {wanted}\n")
    ...
    for package in packages:
        ...
        for version, url in releases:
            for name, digest in tarball_hashes(package, version, url).items():
                if digest == wanted:
    ...
    if not matches:
        ...
        print(f"\nThe file is byte-identical in {len(matches)} releases, so record the lowest,")
