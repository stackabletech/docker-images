...
def find_reporting_task(name: str, port: str):
    ...
    try:
        ...
        raise Exception(
            "Failed to retrieve ReportingTask[{}/{}]: {}".format(name, port, str(ex))
        ) from None
    ...
    for task in reporting_tasks:
        ...
        task_port = task_component["properties"][
...
def set_reporting_task_running(task):
    ...
    try:
        ...
        raise Exception(
            "Failed to set ReportingTask [{}] to RUNNING: {}".format(task.id, str(ex))
        ) from None
...
def main():
    ...
    all_args.add_argument(
        "-n", "--nifi_api_url", required=True, help="The NiFi node url to connect to."
    )
    all_args.add_argument(
        "-u", "--username", required=True, help="Username to connect as."
    )
    all_args.add_argument(
