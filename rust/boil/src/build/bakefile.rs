...
// impl Graph {
//     pub fn all() -> Self {
//         let image_config_paths: Vec<_> = glob("./**/boil-config.toml")
//             .expect("glob pattern must be valid")
//             .filter_map(Result::ok)
//             .collect();
...
//         for image_config_path in image_config_paths {
//             let image_config = ImageConfig::from_file(&image_config_path).unwrap();

//             let (image_name, _) = image_config_path
//                 .to_str()
//                 .unwrap()
//                 .rsplit_once('/')
//                 .unwrap();

//             let pairs = image_config.filter_by_version(None).unwrap();

//             targets.insert_targets(image_name.to_owned(), pairs);
//         }
...
//         for VersionOptionsPair { version, options } in pairs {
//             let key = format!("{image_name}:{version}");
//             let child_nodes = Vec::new();

//             // let nodes = self.insert_targets(image_name, pairs);
//         }
//     }
// }
